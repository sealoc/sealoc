...
def test_load_data_access_layer_with_explicit_url(tmp_path: Path) -> None:
    """load_data_access_layer returns a DAL with a live engine when given an explicit database URL."""
    url = _make_db(tmp_path)
    dal = load_data_access_layer(database_url=url)

    assert isinstance(dal, DataAccessLayer)
    assert isinstance(dal.engine, db.Engine)


def test_load_data_access_layer_with_env_fallback(monkeypatch, tmp_path: Path) -> None:
    """load_data_access_layer resolves the database URL from SEALOC_DATABASE_URL when no argument is given."""
    url = _make_db(tmp_path)
    monkeypatch.setenv("SEALOC_DATABASE_URL", url)

    dal = load_data_access_layer()

    assert isinstance(dal, DataAccessLayer)
    assert isinstance(dal.engine, db.Engine)


def test_load_data_access_layer_raises_without_database(
    monkeypatch, tmp_path: Path
) -> None:
    """load_data_access_layer raises ValueError when no URL is provided and env var is unset."""
...
def test_load_data_access_layer_without_image_store(tmp_path: Path) -> None:
...
def test_load_data_access_layer_with_image_store(tmp_path: Path) -> None:
    """dal.image_store is an ImageStore when a valid image_dir is provided."""
    url = _make_db(tmp_path)
    image_dir = tmp_path / "images"
    image_dir.mkdir()

    dal = load_data_access_layer(database_url=url, image_dir=image_dir)

    assert isinstance(dal.image_store, ImageStore)


def test_load_data_access_layer_image_store_env_fallback(
    monkeypatch, tmp_path: Path
) -> None:
...
def test_load_data_access_layer_image_store_absent_without_env_or_arg(
    tmp_path: Path,
) -> None:
